"""
This a PQ class from internet with slight modification
    ...
    *amortized due to occasional resizing of the underlying python list
    ...
    class Locator:
        ...
        def __init__(self, value, item, i):
            #self._value is a tuple
            self._value = value
            self._item = item
            self._index = i

        def __eq__(self, other):
            return self._value[0] == other._value[0] and self._value[1] == other._value[1]
    ...
    def append(self, item):
        ...
        if item.obj not in self._set:
            self.size += 1
            self._set.add(item.obj)
            ...
            for j in range(len(self._pq)):
                if self._pq[j]._item.obj == item.obj:
                    self.update(self._pq[j], item.rank, item)
                    break
